var initialRunTestDataService = function($q, TestCaseService, HostService) {
	var categories = [];
	return TestCaseService.getTestCategories().success(function(data, status, headers) {
		...
		if(angular.isArray(data)) {
			...
			$q.all([testInstancePromise, hostDataPromise]).then(function(resp){
				return {
					testcategories: categories,
					testinstances: resp[0].data,
					hosts: resp[1].data
				};
			});
		}
...
mycontrollers.RunTestController = function($scope, $modal, TestCaseService, HostService, TestExecutorService, TestResultService, MessageService) {
